...
allocate(deltaKronecker_ai(nWalls,nWalls))
...
allocate(Wall_TempOld_ad(nWalls,SC_NoOfNodes_i), &
	Wall_Temp_ad(nWalls,SC_NoOfNodes_i), &
	Wall_X_ad(nWalls,SC_NoOfNodes_i))

allocate(shapeFactors_ad(nWalls,nWalls), &
	matrixA_ad(nWalls,nWalls))

allocate(wallTemp_ad(nWalls), &
	wallEmiss_ad(nWalls), &
	matrixC_ad(nWalls), &
	wallHeatFlux_ad(nWalls),&
	wallHeatFluxOut_ad(nWalls),&
	SC_ThermCond_ad(nWalls),SC_Rho_ad(nWalls), &
	SC_Cp_ad(nWalls), &
	heat_inner_walls(nWalls), &
	Walls_StoredEnergy_ad(nWalls),&
	Walls_outHeat_ad(nWalls), &
	GasHTC_ad(nWalls), &
	Wall_Area_ad(nWalls),&
	Wall_thk_ad(nWalls))
